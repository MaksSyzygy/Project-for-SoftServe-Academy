...
namespace Diplom
{
    class CreateClient : INewClient//Создание нового клиента в БД
    {
        ...
        public void AddNewCLient()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connStr))
            {
                ...
                {
                    query = $"insert into Clients(ClientID, ClientName, ClientSurname, ClientBirthday, ClientPhone, " +
                        $"ManagerID) values({item.ClientID}, '{item.Name}', '{item.Surname}', '{item.Birthday}', " +
                        $"'{item.Phone}', {item.Manager})";
                }
            ...
            using (SqlConnection connection = new SqlConnection(connStr))
            {
                ...
                {
                    query = $"insert into ClientInfo(ID, Balance, Credit, Deposit, Password) " +
                        $"values({item.InfoID}, {item.Balance}, {item.Credit}, {item.Deposit}, '{item.Password}')";
                }
